COM_CAB_SESSION = "JSESSIONID="
COM_FIN_SESSION = "; Path=/CL;"
...
def extraer_datos_session(response):
    session = extraer_texto(
        response.headers["Set-Cookie"], COM_CAB_SESSION, COM_FIN_SESSION)
    # print(session)

    cookie_barracuda = extraer_texto(
        response.headers["Set-Cookie"], COM_CAB_COOKIE_BARRACUDA, COM_FIN_COOKIE_BARRACUDA)
    # print(cookie)

    viewstate = extraer_texto(
        response.text, COM_CAB_VIEWSTATE, COM_FIN_VIEWSTATE)
    # print(viewstate)
    cookie = extraer_texto(
        response.headers["Set-Cookie"], COM_CAB_COOKIE, COM_FIN_COOKIE)

    return session, cookie_barracuda, viewstate, cookie
