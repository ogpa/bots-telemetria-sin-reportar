...
import numpy as np
import os
from Comsatel.mes_anho_comsatel import mes_anho_comsatel
...
COM_URL_DISTANCIADIARIA = "http://clreportes.comsatel.com.pe/CLReporte/faces/page/distanciadiariamensual/distanciaDiariaMensualListar.xhtml"
...
def scan_comsatel(hoy):
    ...
    cookieBarracuda_LogIn = extraer_texto(
        respLogin.headers["Set-Cookie"], BNI + "BARRACUDA_LB_COOKIE=", "; Path=/; HttpOnly")
    ...
    headers_Found = {
        ...
        'Cookie': 'JSESSIONID=' + session_LogIn + '; ' + BNI + 'BARRACUDA_LB_COOKIE=' + cookieBarracuda_LogIn,
    ...
    # print(respCLReporte.headers)
    cookieBarracuda_CLReporte = extraer_texto(
        respCLReporte.headers["Set-Cookie"], BNI + "BARRACUDA_LB_COOKIE=", "; Path=/; HttpOnly")
    ...
    headers_preReporte = {
        ...
        'Connection': 'keep-alive',
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'Cookie': 'JSESSIONID=' + session_CLReporte + ";" + cookie_prePopUpReporte + '; ' + BNI + 'BARRACUDA_LB_COOKIE=' + cookieBarracuda_CLReporte,
        'Faces-Request': 'partial/ajax',
    ...
    headers_ventanaReporte = {
        ...
        'Accept-Language': 'en',
        'Connection': 'keep-alive',
        'Cookie': 'JSESSIONID=' + session_CLReporte + '; ' + BNI + 'BARRACUDA_LB_COOKIE=' + cookieBarracuda_CLReporte,
        'Referer': COM_URL_CLREPORTE,
        'Upgrade-Insecure-Requests': '1',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36'
    }

    respventanaDistanciaDiaria = requests.request(
        "GET", COM_URL_DISTANCIADIARIA, headers=headers_ventanaReporte, data=payload_ventanaReporte)

    # Hasta acá está bien
    # print(respventanaDistanciaDiaria.headers)
    # print(respventanaDistanciaDiaria.text)
    # Aquí debe decir "No cuenta con registros."
    # De aquí debo sacar un nuevo viewstate
    viewstate_DistanciaDiaria = extraer_texto(
        respventanaDistanciaDiaria.text, 'id="j_id1:javax.faces.ViewState:0" value="', '" autocomplete="off"')
    # print(viewstate_DistanciaDiaria)
    ...
    payload_PreReporteDistanciaDiaria = 'j_idt44%3AfrmBusquedaAvanzada=j_idt44%3AfrmBusquedaAvanzada&j_idt44%3AtxtPlaca=&j_idt44%3AtxtNroMotor=&j_idt44%3AtxtCodigoExterno=&j_idt44%3AcboFlota=0&j_idt44%3AcboCompania=0&j_idt44%3AcboSubFlota=0&j_idt44%3Aj_idt59=' + \
        "2022" + '&j_idt44%3AcboMeses=' + "NOVIEMBRE" + '&javax.faces.ViewState=' + urllib.parse.quote(
            viewstate_DistanciaDiaria, safe="") + '&j_idt44%3AfrmBusquedaAvanzada%3AbtnBuscar=j_idt44%3AfrmBusquedaAvanzada%3AbtnBuscar'
    print(payload_PreReporteDistanciaDiaria)
    headers_PreReporteDistanciaDiaria = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
        ...
        'Content-Type': 'application/x-www-form-urlencoded',
        'Cookie': 'JSESSIONID=' + session_CLReporte + '; ' + BNI + 'BARRACUDA_LB_COOKIE=' + cookieBarracuda_CLReporte,
        'Origin': 'http://clreportes.comsatel.com.pe',
        'Referer': COM_URL_DISTANCIADIARIA,
        'Upgrade-Insecure-Requests': '1',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36'
    }

    respPreReporteDistanciaDiaria = requests.request(
        "POST", COM_URL_DISTANCIADIARIA, headers=headers_PreReporteDistanciaDiaria, data=payload_PreReporteDistanciaDiaria)
    # print(respPreReporteDistanciaDiaria)
    # print(respPreReporteDistanciaDiaria.headers)
    # print(respPreReporteDistanciaDiaria.text)
    payload_ReporteDistanciaDiaria = {}
    headers_ReporteDistanciaDiaria = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
        'Accept-Language': 'en-US,en;q=0.9,es;q=0.8,it;q=0.7',
        'Cache-Control': 'max-age=0',
        'Connection': 'keep-alive',
    ...
    respReporteDistanciaDiara = requests.request("GET", COM_URL_DISTANCIADIARIA,
                                                 headers=headers_ReporteDistanciaDiaria, data=payload_ReporteDistanciaDiaria)

    # print(respReporteDistanciaDiara.text)
    # Aquí continuaba
    ...
    # df = df[1:].iloc[:, [1, 4, 5, 11, 12]]
    # df = df.rename(
    #     columns={df.columns[4]: "Fecha Ultima Localizacion", df.columns[5]: "Direccion"})
    # df = df.drop(10)
