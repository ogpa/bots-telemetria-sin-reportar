...
def login_hunter_pro():
    ...
    payload_Loginv3 = "LanguageOptionId=82&username=mbrenting.dpizarro&password=mbrenting2022!&action=log+on&"
    headers_Loginv3 = {
    ...
    response_Loginv3 = requests.request("POST", HUN_URL_LOGINV3, headers=headers_Loginv3, data=payload_Loginv3)
    
    #print(response_Loginv3.headers["Set-Cookie"])
    
    d_Loginv3 = extraer_datos_session(response_Loginv3)
    ...
    response_Config = requests.request("GET", HUN_URL_CONFIG, headers=headers_Config, data=payload_Config)
    ...
    response_Live = requests.request("GET", HUN_URL_LIVE, headers=headers_Live, data=payload_Live)
    
    #print(response_Live.text)
    
    cookie_asp = extraer_texto(response_Live.headers["Set-Cookie"],CABECERA_ASPNET,FIN_ASPNET)
    # vsg_Login = d_Login[2]
    # ev_Login = d_Login[3]
